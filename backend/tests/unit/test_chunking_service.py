...
def user_id(db_session: Session) -> str:
    return auth_service.create_user(db_session, "chunking-owner@example.com", "hunter22").id


def _make_document(db_session: Session, user_id: str, filename: str, content: bytes) -> Document:
    document = Document(
        user_id=user_id,
        name=filename,
...
def _run(
    ...
    resolved = service.resolve_run(
        db_session, user_id, document_id=document.id, chunk_size=chunk_size, strategy=strategy, overlap=overlap
    )
...
def test_stream_chunking_caps_at_200_and_reports_true_total(
    ...
    document = _make_document(db_session, user_id, "long.pdf", make_words_pdf(500))

    events = _run(db_session, user_id, document, 1)
...
def test_stream_chunking_no_extractable_text_reports_failure(
    ...
    document = _make_document(db_session, user_id, "empty.pdf", make_words_pdf(0))

    events = _run(db_session, user_id, document, 10)
...
def test_resolve_run_unregistered_strategy_raises_value_error(
    ...
    document = _make_document(db_session, user_id, "report.pdf", make_words_pdf(10))

    with pytest.raises(ValueError):
        service.resolve_run(
            db_session, user_id, document_id=document.id, chunk_size=5, strategy="semantic"
        )
...
def test_resolve_run_invalid_chunk_size_raises_value_error(
    ...
    with pytest.raises(ValueError):
        service.resolve_run(
            db_session, user_id, document_id=document.id, chunk_size=0, strategy="fixed-size"
        )


def test_resolve_run_unknown_document_raises_file_not_found(db_session: Session, user_id: str) -> None:
...
def test_stream_chunking_uncapped_result_has_matching_total_and_length(
    ...
    document = _make_document(db_session, user_id, "short.pdf", make_words_pdf(20))

    events = _run(db_session, user_id, document, 5)
...
def test_stream_chunking_emits_nondecreasing_progress_across_pages(
    ...
    document = _make_document(
        db_session, user_id, "multi.pdf", make_multi_page_words_pdf([10, 10, 10, 10])
    )
...
def test_stream_chunking_emits_at_least_one_progress_event_for_single_page(
    ...
    document = _make_document(db_session, user_id, "single.pdf", make_words_pdf(20))
...
def test_resolve_run_overlap_equal_to_chunk_size_raises_value_error(
    ...
    with pytest.raises(ValueError):
        service.resolve_run(
            db_session, user_id, document_id=document.id, chunk_size=5, strategy="fixed-size", overlap=5
        )
...
def test_resolve_run_overlap_greater_than_chunk_size_raises_value_error(
    ...
    with pytest.raises(ValueError):
        service.resolve_run(
            db_session, user_id, document_id=document.id, chunk_size=5, strategy="fixed-size", overlap=10
        )
...
def test_resolve_run_negative_overlap_raises_value_error(
    ...
    with pytest.raises(ValueError):
        service.resolve_run(
            db_session, user_id, document_id=document.id, chunk_size=5, strategy="fixed-size", overlap=-1
        )
...
def test_stream_chunking_higher_overlap_increases_total_chunks(
    ...
    document = _make_document(db_session, user_id, "long.pdf", make_words_pdf(100))

    no_overlap_events = _run(db_session, user_id, document, 10, overlap=0)
    overlap_events = _run(db_session, user_id, document, 10, overlap=5)
...
def test_stream_chunking_result_echoes_overlap(db_session: Session, user_id: str) -> None:
    document = _make_document(db_session, user_id, "short.pdf", make_words_pdf(20))

    events = _run(db_session, user_id, document, 5, overlap=2)
...
def test_stream_chunking_does_not_persist_chunks_for_the_document(
    ...
    _run(db_session, user_id, document, 5)
...
def test_stream_chunking_repeated_runs_never_persist_chunks(
    ...
    _run(db_session, user_id, document, 5)
    _run(db_session, user_id, document, 10, overlap=2)
    _run(db_session, user_id, document, 3)
...
def test_save_chunks_stream_emits_progress_before_the_terminal_result(
    ...
    events = _save(db_session, user_id, document, 5)
...
def test_save_chunks_resave_replaces_previous_saved_chunks(
    ...
    _save(db_session, user_id, document, 5)
    _save(db_session, user_id, document, 10)
...
def test_save_chunks_no_extractable_text_persists_nothing(
    ...
    events = _save(db_session, user_id, document, 10)
    response = events[-1][1]
...
def test_save_chunks_failed_extraction_leaves_prior_saved_chunks_untouched(
    ...
    good = _make_document(db_session, user_id, "short.pdf", make_words_pdf(20))
    _save(db_session, user_id, good, 5)

    empty = _make_document(db_session, user_id, "empty.pdf", make_words_pdf(0))
    _save(db_session, user_id, empty, 10)
