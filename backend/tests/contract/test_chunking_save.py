...
def test_save_chunking_success_persists_and_returns_result(
...
def test_save_chunking_resave_replaces_previous_saved_chunks(
    client: TestClient, corpus_id: str, db_session: Session
) -> None:
    document_id = upload_pdf(client, corpus_id, "report.pdf", make_words_pdf(20))
    ...
    client.post(
        "/api/chunking/save", json={"documentId": document_id, "chunkSize": 10, "overlap": 2}
    )
...
def test_save_chunking_extraction_failed_persists_nothing(
    client: TestClient, corpus_id: str, db_session: Session
) -> None:
...
def test_save_chunking_invalid_chunk_size(client: TestClient, corpus_id: str) -> None:
    document_id = upload_pdf(client, corpus_id, "report.pdf", make_words_pdf(20))

    response = client.post(
        "/api/chunking/save", json={"documentId": document_id, "chunkSize": 0}
    )
...
def test_save_chunking_overlap_equal_to_chunk_size_is_rejected(
    ...
    response = client.post(
        "/api/chunking/save",
        json={"documentId": document_id, "chunkSize": 5, "overlap": 5},
    )
...
def test_save_chunking_unknown_document(client: TestClient) -> None:
    response = client.post(
        "/api/chunking/save",
        json={"documentId": "00000000-0000-0000-0000-000000000000", "chunkSize": 5},
    )
